"""
Django settings for dmoj project (actualizado para Django 5.2).

Notas:
- Migrado para ser compatible con Django 5.2 (LTS).
- Cambios principales: BASE_DIR con pathlib, DEFAULT_AUTO_FIELD actualizado a BigAutoField,
  INSTALLED_APPS / MIDDLEWARE como listas, eliminación de USE_L10N (deprecado), y se añade
  SecurityMiddleware al tope de la pila de middlewares.

Revisa las dependencias de terceros (social-auth, mptt, reversion, martor, etc.) y ejecuta
`python manage.py check` y `python manage.py test` en un entorno con Python 3.12.
"""
...
MIDDLEWARE = [
    # SecurityMiddleware should be near top
